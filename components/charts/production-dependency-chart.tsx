...
interface ProductionDependencyChartProps {
    ...
    <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700">
      ...
      <div className="space-y-1.5 text-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-slate-600 dark:text-slate-300">Economía Petrolera:</span>
          <span className="font-mono font-bold text-slate-900 dark:text-white ml-auto">
            ~{data?.pibPorcentaje}% del PIB
          </span>
        </div>
...
export default function ProductionDependencyChart({ data }: ProductionDependencyChartProps) {
  ...
  const chartData = (data ?? [])?.map(item => ({
    ...
    pibPorcentaje: parseFloat(item?.pib_petrolero_categoria ?? '0'),
    ...
    <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800">
      <CardHeader className="px-4 md:px-6">
        <CardTitle className="text-base md:text-lg text-slate-900 dark:text-white flex items-center gap-2">
          ...
          <span className="leading-tight">Dependencia Petrolera</span>
        </CardTitle>
        <p className="text-xs md:text-sm text-slate-600 dark:text-slate-400 mt-1">
          Producción diaria vs % del PIB
        </p>
      </CardHeader>
        ...
        <ResponsiveContainer width="100%" height={350} className="md:h-[450px]">
          ...
          >
            ...
            <YAxis 
              ...
              stroke={tickColor}
              label={{ 
                value: 'Economía Petrolera (% del PIB)', 
                angle: 90, 
            ...
            <Line 
              yAxisId="right"
              type="monotone" 
              dataKey="pibPorcentaje" 
              name="Economía Petrolera (% del PIB)"
